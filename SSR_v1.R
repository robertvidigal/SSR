...
ssr$mode<-ifelse(ssr$ONDA==1, 1,
                 ifelse(ssr$ONDA==2,0,
                        ifelse(ssr$ONDA==3,0,1)))
table(ssr$mode)
table(ssr$op_previdência)
table(ssr$op_previdência_AFAVOR)
describe(ssr$bolsonaro) # Higher values indicate more negative views. Ok.
...
# GENDER EFFECTS
summary(glm(op_previdência_AFAVOR~tratamento+mulher, family="binomial", data=w1)) 
summary(glm(op_previdência_AFAVOR~tratamento+mulher, family="binomial", data=w2))
summary(glm(op_previdência_AFAVOR~tratamento+mulher, family="binomial", data=w3))
summary(glm(op_previdência_AFAVOR~tratamento+mulher, family="binomial", data=w4))
...
# MODELO PARCIMONIOSO TABLE 1
model1<-lm(op_previdência_AFAVOR~tratamento+bolsonaro+mulher+idade+ONDA+
             tratamento*ONDA+tratamento*mulher, family="binomial", data=ssr)
summary(model1) # run as lm() for fit stats

stargazer::stargazer(model1, type="text")

# ANTIBOLSO / IDEOLOGY
summary(glm(op_previdência_AFAVOR~tratamento+antibolso+mulher+idade+ONDA+
              ONDA*tratamento+
              tratamento*antibolso+
              ONDA*tratamento*antibolso, family="binomial", data=ssr)) # No Effects on the triple interaction.

# A MULTILEVEL MODEL
# 2nd-level: Survey Environment (FIXED EFFECTS)
require(plm)
...
jpeg(filename="~/Dropbox/PhD/ARTIGOS Trabalhando/SSR/meansplot.png", units="in", width = 5, height = 4, res=300)
ggplot(tab2, aes(x=ONDA, y=Perc, group=tratamento, shape=tratamento, color=tratamento))+ 
  ...
  theme(legend.title = element_blank(), axis.text.x = element_text(face='bold', size=12))
dev.off()
...
### PLOT OLD TABLE 2 INTO FIGURE 2
